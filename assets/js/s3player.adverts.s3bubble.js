...
(function ($) {
	...
	$.fn.mediaPlayer = function(extras) {
		...
		var HTMLContent = $('<div class="playerScreen">\
			...
			<a tabindex="1" href="#" class="skipAdvert">Skip advert</a>\
			</div>\
			<div class="controls">\
			<div class="progress">\
			<div class="seekBar">\
		...
		if ('hd' in settings.media) {

			var playerHD = settings.media.hd;
			var playerSD = settings.media.sd;

			$(HTMLContent).find('.hd-video').addClass('enabled');
			$(HTMLContent).find('.hd-video').on('click', function() {

				var currentPlay = $(playerID).data("jPlayer").status.currentTime;

				if ($(this).hasClass('disabled')) {

					$(this).removeClass('disabled');
					$(playerID).jPlayer('setMedia', playerHD);
					$(playerID).jPlayer('play', currentPlay);

				} else {

					$(this).addClass('disabled');
					$(playerID).jPlayer('setMedia', playerSD);
					$(playerID).jPlayer('play', currentPlay);
			...
			settings.media = settings.media.sd;

		} else {

			$(HTMLContent).find('.hd-video').addClass('disabled');
		...
		if ($(this).hasClass('audioPlayer')) { 
			...
			$(this).find('.smallscreen').remove();
			$(this).find('.controls').css('bottom', '0px');
			settings.size.height = '45px';
		}
	...
	function createPlayer(playerGUI, mainPlayer, settings, extras) {
		...
		var options = {
			...
			volumechange: function(event) {        
				...
				setcookie('jplayer-volume', event.jPlayer.options.volume, 386);  
			},
